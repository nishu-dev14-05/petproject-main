...
export default function GeneralChat({ darkMode = false }: GeneralChatProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Use 'general' as breed for general dog questions
      const response = await petPalAPI.askChatbot('general', input);
    ...
    <div className={`rounded-xl shadow-lg overflow-hidden ${darkMode ? 'bg-gray-800' : 'bg-white'}`}>
      <div className={`p-4 ${
        ...
        <h3 className="text-xl font-bold text-white flex items-center gap-2">
          ...
          General Dog Care Assistant
        </h3>
        <p className="text-white/80 text-sm mt-1">
          Ask me anything about dogs, their care, training, health, or nutrition!
        </p>
      </div>
      ...
      <div className="h-96 overflow-y-auto p-4 space-y-4">
        {messages.length === 0 ? (
          <div className={`text-center mt-20 ${darkMode ? 'text-gray-400' : 'text-gray-500'}`}>
            ...
            <p className="font-medium mb-2">Welcome to the General Dog Care Assistant!</p>
            <p className="text-sm mt-2 px-4">
              I can help you with general questions about dogs such as:
            </p>
            <div className={`mt-4 text-sm space-y-1 ${darkMode ? 'text-gray-400' : 'text-gray-600'}`}>
              <p>• "What are the best dog breeds for families?"</p>
              <p>• "How often should I walk my dog?"</p>
              <p>• "What foods are toxic to dogs?"</p>
              <p>• "How do I train a puppy?"</p>
              <p>• "What are signs of a healthy dog?"</p>
            </div>
          </div>
        ) : (
      ...
      <form onSubmit={handleSubmit} className={`border-t p-4 ${darkMode ? 'border-gray-700' : 'border-gray-200'}`}>
        <div className="flex gap-2">
          <input
            ...
            placeholder="Ask a question about dogs..."
